...
public class writeTestcasetoTFS extends connectivityToTFS
{
	...
	//	I need to fetch the date from excel and no of rows
		...
		{	
			...
			int requirementIDcol = 0;
			
			if(sheet.getRow(i+1).getCell(requirementIDcol).getCellType() == Cell.CELL_TYPE_BLANK)
			{
		...
		//	newWorkItem.getFields().getField("Severity").setValue(Severity);
			...
			newWorkItem.getFields().getField("STEPS").setValue(TestSteps);
